...
class FbController extends UtilBaseController
{
    ...
    public function fbCallbackAction()
    {
        ...
        try {
            ...
            $FbCommunicationIssueMsgs[] = "Error Code: ".$e->getLine();
        ...
        try {
            ...
            $entity->setFbAccessToken($accessToken->getValue());
            $entity->setFbUserId($tokenMetadata->getUserId());
            $entity->setFbAccessTokenExpireTs($accessToken->getExpiresAt());

            $entity = $entityManager->merge($entity);
            $entityManager->flush();

            //Log in by FB access token
            $user = $this->logUserInByCredential($entity->getEmail(), $entity->getFbAccessToken());

            if($user !== false) {

                //All good, send them to user page
                $this->flashMessengerMulti([sprintf('Welcome %s.',$user->getDisplayName()),'You are now logged in.'],'success');
    ...
    //@todo use, relocate or remove shareProgressOnFacebook($fb, $accessToken)
